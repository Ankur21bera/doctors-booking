  ...
  const getAvailableSlots = async () => {
    ...
    for(let i =0 ; i < 7; i++) {
     ...
     while(currentDate < endTime) {
       ...
       const isSlotAvailable = doctInfo.slots_booked[slotDate] && doctInfo.slots_booked[slotDate].includes(slotTime) ? false : true
  ...
  return doctInfo && (
    <div>
      <div className='flex flex-col sm:flex-row gap-4'>
        <div>
          <img className='bg-[#5f6fff] w-full sm:max-w-72 rounded-lg' src={doctInfo.image} alt="" />
        </div>
        ...
        <div className='flex-1 border border-gray-400 rounded-lg px-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          <p className='text-gray-500 font-medium mt-4'>
            Appointment Fees: <span className='text-gray-600'>{currencySymbol}{doctInfo.fees}</span>
          </p>
        </div>
      </div>
      ...
      <div className='flex items-center gap-2 w-full overflow-x-hidden mt-2'>
      ...
      <button onClick={bookAppointment} className='bg-[#5f6fff] cursor-pointer text-white text-sm font-light px-14 py-3 rounded-full my-6'>Book Appointment</button>
      </div>
